...
class MySql(SqlBase):
    ...
    def insert_proxy(self, table_name, proxy):
        try:
            ...
            data = ( proxy.ip, proxy.port, proxy.country, proxy.anonymity,
                    proxy.https, proxy.speed, proxy.source,proxy.status)

            print(data)

            command = ("INSERT  INTO {} "
                       "( ip, port, country, anonymity, https, speed, source, status)"
                       "VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')".format(table_name)) % (data)
            print(command,'\n','-----------')
            self.cursor.execute(command)
            return True
        except Exception as e:
    ...
    def update_proxy(self, table_name, proxy):
        try:
            command = "UPDATE {table_name} set https='{https}', speed={speed}, " \
                      "status={status}, anonymity = {anonymity},save_time={save_time} " \
                      "where id={id};".format(
                table_name=table_name, https=proxy.https,
                speed=proxy.speed, id=proxy.id, status=proxy.status, anonymity=proxy.anonymity,
                save_time='NOW()')
    ...
    def vaildator_ip(self, table_name):
        '''验证ip的有效性,并且更改状态1有效O无效'''
        try:
            sql = '''Select ip, port, country, anonymity, https, speed, source, status ,id From {} WHERE status=1'''.format(table_name)
            ips = self.query(sql)
            print('开始检测ip是否有效',len(ips),datetime.datetime.now())
            for ip in ips:
                try:
                    '''利用telnetlib来验证ip,如果超出10s则判定为无效'''
                    telnetlib.Telnet('{ip}'.format(ip=ip[0]),port='{port}'.format(port=ip[1]),timeout=10)
                    sql = '''UPdate {table_name} SET status={status} WHERE id={id}'''.format(table_name=table_name,                                                                status=1, id=ip[-1])
                    self.cursor.execute(sql)
                    self.conn.commit()
                except:
                    sql = '''DELETE FROM {table_name} WHERE id={id}'''.format(table_name=table_name,
                                                                                              id=ip[-1])
                    self.cursor.execute(sql)
                    self.conn.commit()
                    continue
            print('本次ip验证完成：',datetime.datetime.now())
        except Exception as e:
            logging.exception('mysql vaildator_ip exception msg:%s' % e)
